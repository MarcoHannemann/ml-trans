...
def predict(
    trained_model: tf.keras.Model(), x: np.ndarray, y: Union[None, np.ndarray]
) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    cp = configparser.ConfigParser(
        delimiters="=", converters={"list": lambda x: [i.strip() for i in x.split(",")]}
    )
    ...
    es_callback = tf.keras.callbacks.EarlyStopping(
        monitor="val_loss", patience=early_stopping_epochs, verbose=1, min_delta=0.001
    )
