...
class Location:
        ...
        if not self.is_scalar:
            if pd.isnull(self.sunrise):
                ...
                try:
                    ...
                    self.local_date = np.where(
                        pd.isnull(self.local_date), np.nan, self.local_date
                    )
    ...
    def _localize_date(self) -> NDArray:
        ...
        return np.array(
            pd.DatetimeIndex(self.date).tz_localize(
    ...
    def local_solar_time(self):
        ...
        self.lst = self.lt + 4 * (self.lon - self.lstm) + self.eot

    def calc_day_angle(self):
        """Calculates the day angle in radians."""
        self.day_angle = (
            2 * np.pi * (np.array([xi.dayofyear for xi in self.local_date]) - 1)
        ) / 365
    ...
    def _sunrise_hour_angle(self) -> NDArray:
        ...
        return np.degrees(
            np.arccos(
                np.cos(np.radians(constants.zenith_constant))
                / (np.cos(np.radians(self.lat)) * np.cos(self.dec))
    ...
    def calc_sunrise_sunset(self):
        ...
        if (
            not (np.isnan(sunrise_decimal[0]))
            and sunrise_share[0] < 1
            and sunset_share[0] < 1
        ):
            self.sunrise = self.local_date[0].replace(
                hour=int(sunrise_decimal[0]),
                minute=int((sunrise_decimal[0] * 60) % 60),
                second=int((sunrise_decimal[0] * 3600) % 60),
            )

            self.sunset = self.local_date[0].replace(
                hour=int(sunset_decimal[0]),
...
def hogan_sza_average(
    lat: float, lon: float, date: pd.Timestamp, timezone: str
) -> float:
